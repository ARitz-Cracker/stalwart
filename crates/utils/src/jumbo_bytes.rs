...
// TODO: Have these appropriately set by stalwart's `Core` after the config schema is modified to set these settings
static SPILLOVER_SIZE: AtomicUsize = AtomicUsize::new(1 * 1024 * 1024);
static TEMPFILE_DIR: LazyLock<RwLock<Arc<Path>>> = LazyLock::new(|| {
    if let Some(dir) = std::env::var("STALWART_BUFFER_DIR").ok() {
        RwLock::new(PathBuf::from(dir).into())
    } else {
        RwLock::new(std::env::temp_dir().into())
    }
});
...
fn new_tempfile_path(dir: &Path) -> PathBuf {
    ...
    new_path.push(format!(
        "stalwart_{pid}_{ecpoch_value:x}_{nonce_value:x}.temp"
    ));
    new_path
}
...
impl JumboBytesMut {
    ...
    pub fn set_spillover_size(size: u32) {
        ...
        SPILLOVER_SIZE.store((size as usize).max(1024 * 1024), Ordering::Relaxed);
    }
    ...
    fn handle_poll_transition(&mut self, cx: &mut AsyncContext<'_>) -> Result<bool> {
        ...
        match future.poll_unpin(cx) {
            ...
            Poll::Pending => {
                // `self.inner` was taken above, so the in-flight future has to be put back. Letting
                // it drop here would cancel the underlying `spawn_blocking` file operation and
                // throw away the waker it registered, leaving the task parked forever.
                self.inner = Some(JumboBytesMutInner::Transitioning {
                    future,
                    original_cursor_pos,
                    cursor_pos,
    ...
    fn poll_write(
        ...
        match &mut self_mut.inner {
            Some(JumboBytesMutInner::Ram { bytes }) => {
                ...
                if buf.len().saturating_add(cursor_pos as usize) > self_mut.max_ram_size {
...
impl Write for JumboBytesMutSync {
    fn write(&mut self, buf: &[u8]) -> Result<usize> {
        match self.inner.as_mut() {
            Some(JumboBytesMutSyncInner::Ram { bytes }) => {
                if buf.len().saturating_add(bytes.position() as usize) > self.max_ram_size {
                    let cursor_pos = bytes.position();
                    ...
                    self.inner = Some(JumboBytesMutSyncInner::File {
                        path: new_path,
                        file,
                        file_size: (bytes.len() as u64).max(cursor_pos + buf_bytes_written as u64),
                    });
            ...
            Some(JumboBytesMutSyncInner::File {
                ...
                if let Ok(cursor_pos) = file.seek(SeekFrom::Current(0))
                    && cursor_pos > *file_size
                {
                    *file_size = cursor_pos;
                }
                bytes_written
            }
            None => Err(reuse_after_drop_error()),
...
mod tests {
    ...
    // `SPILLOVER_SIZE` is process-global, so tests that reconfigure it must not overlap.
    static SPILLOVER_LOCK: tokio::sync::Mutex<()> = tokio::sync::Mutex::const_new(());

    async fn lock_spillover(size: u32) -> tokio::sync::MutexGuard<'static, ()> {
        let guard = SPILLOVER_LOCK.lock().await;
        JumboBytesMut::set_spillover_size(size);
        guard
    }

    #[tokio::test]
    async fn spillover_size_is_a_floor_not_a_ceiling() {
        // Below the hard-coded 1MiB minimum: the minimum wins and the data stays in RAM.
        let _guard = lock_spillover(1024).await;
        let mut bytes = JumboBytesMut::new(u64::MAX);
        bytes.write_all(&vec![0u8; 512 * 1024]).await.unwrap();
        assert!(
            bytes.as_slice().is_some(),
            "512KiB must stay in RAM: 1KiB is below the 1MiB floor"
        );

        // Above the minimum: the configured value is honoured rather than clamped down to 1MiB.
        JumboBytesMut::set_spillover_size(8 * MIB as u32);
        let mut bytes = JumboBytesMut::new(u64::MAX);
        bytes.write_all(&vec![0u8; 4 * MIB]).await.unwrap();
        assert!(
            bytes.as_slice().is_some(),
            "4MiB must stay in RAM when the spillover size is 8MiB"
        );
    }

    #[tokio::test]
    async fn spills_to_disk_and_round_trips() {
        let _guard = lock_spillover(MIB as u32).await;
        let mut bytes = JumboBytesMut::new(u64::MAX);
        for _ in 0..8 {
            bytes.write_all(&vec![0xAB; 256 * 1024]).await.unwrap();
        }

        assert_eq!(bytes.len(), 2 * MIB as u64);
        assert!(bytes.as_slice().is_none(), "should have spilled to disk");
        assert!(bytes.take_vec().is_none(), "spilled data has no inline Vec");

        let round_tripped = bytes.into_vec(u64::MAX).await.unwrap();
        assert_eq!(round_tripped.len(), 2 * MIB);
        assert!(round_tripped.iter().all(|&b| b == 0xAB));
    }

    #[tokio::test]
    async fn into_vec_rejects_data_over_max_size() {
        let _guard = lock_spillover(MIB as u32).await;
        let mut bytes = JumboBytesMut::new(u64::MAX);
        bytes.write_all(&vec![0u8; 2 * MIB]).await.unwrap();

        let err = bytes.into_vec(MIB as u64).await.unwrap_err();
        assert_eq!(err.kind(), IoErrorKind::FileTooLarge);
    }

    #[tokio::test]
    async fn writes_past_max_size_are_rejected() {
        let _guard = lock_spillover(MIB as u32).await;
        let mut bytes = JumboBytesMut::new(64);
        bytes.write_all(&[1u8; 64]).await.unwrap();

        let err = bytes.write_all(&[2u8; 1]).await.unwrap_err();
        assert_eq!(err.kind(), IoErrorKind::FileTooLarge);
        assert_eq!(
            bytes.len(),
            64,
            "a rejected write must not change the length"
        );
    }

    #[tokio::test]
    async fn seek_and_read_span_the_ram_to_disk_transition() {
        let _guard = lock_spillover(MIB as u32).await;
        let mut bytes = JumboBytesMut::new(u64::MAX);
        // Straddle the boundary so the marker lands on either side of the spill.
        bytes.write_all(&vec![0x11; MIB - 4]).await.unwrap();
        bytes.write_all(b"HERE").await.unwrap();
        bytes.write_all(&vec![0x22; MIB]).await.unwrap();
        assert!(bytes.as_slice().is_none(), "should have spilled to disk");

        bytes.seek(SeekFrom::Start(MIB as u64 - 4)).await.unwrap();
        let mut marker = [0u8; 4];
        bytes.read_exact(&mut marker).await.unwrap();
        assert_eq!(
            &marker, b"HERE",
            "data written before the spill must survive it"
        );

        bytes.seek(SeekFrom::End(-1)).await.unwrap();
        let mut last = [0u8; 1];
        bytes.read_exact(&mut last).await.unwrap();
        assert_eq!(last[0], 0x22);
    }

    #[tokio::test]
    async fn overwriting_in_place_after_spilling_does_not_grow_len() {
        let _guard = lock_spillover(MIB as u32).await;
        let mut bytes = JumboBytesMut::from(vec![7u8; 2 * MIB]);
        assert_eq!(bytes.len(), 2 * MIB as u64);

        // Seek far enough forward to trip the spillover, then overwrite bytes that already exist.
        // The buffer is not getting any longer, so neither should `len()`.
        bytes.seek(SeekFrom::Start(MIB as u64)).await.unwrap();
        bytes.write_all(&[1, 2, 3]).await.unwrap();

        assert_eq!(
            bytes.len(),
            2 * MIB as u64,
            "overwriting existing bytes must not extend the length"
        );
        assert_eq!(bytes.into_vec(u64::MAX).await.unwrap().len(), 2 * MIB);
    }

    #[tokio::test]
    async fn sync_overwriting_in_place_after_spilling_does_not_grow_len() {
        let _guard = lock_spillover(MIB as u32).await;
        let mut bytes = JumboBytesMutSync::from(vec![7u8; 2 * MIB]);
        assert_eq!(bytes.len(), 2 * MIB as u64);

        Seek::seek(&mut bytes, SeekFrom::Start(MIB as u64)).unwrap();
        Write::write_all(&mut bytes, &[1, 2, 3]).unwrap();
        ...
        assert_eq!(bytes.into_vec(u64::MAX).unwrap().len(), 2 * MIB);
    }

    #[tokio::test]
    async fn sync_len_tracks_appends_across_the_spill() {
        // This is the backup path: `len()` becomes the leb128 prefix written ahead of the payload,
        // so a stale value silently corrupts the archive.
        let _guard = lock_spillover(MIB as u32).await;
        let mut bytes = JumboBytesMutSync::new(u64::MAX);
        for _ in 0..8 {
            Write::write_all(&mut bytes, &vec![0xCD; 256 * 1024]).unwrap();
        }

        assert_eq!(bytes.len(), 2 * MIB as u64);

        Seek::rewind(&mut bytes).unwrap();
        let mut drained = Vec::new();
        Read::read_to_end(&mut bytes, &mut drained).unwrap();
        assert_eq!(
            drained.len() as u64,
            2 * MIB as u64,
            "the bytes actually readable must match the reported len()"
        );
    }

    #[tokio::test]
    async fn converting_between_sync_and_async_preserves_contents() {
        let _guard = lock_spillover(MIB as u32).await;
        let mut bytes = JumboBytesMut::new(u64::MAX);
        bytes.write_all(&vec![0x5A; 2 * MIB]).await.unwrap();
        bytes.rewind().await.unwrap();

        let mut sync_bytes = JumboBytesMutSync::from(bytes);
        assert_eq!(sync_bytes.len(), 2 * MIB as u64);
        let mut head = [0u8; 8];
        Read::read_exact(&mut sync_bytes, &mut head).unwrap();
        assert_eq!(head, [0x5A; 8]);

        let mut async_bytes = JumboBytesMut::from(sync_bytes);
        assert_eq!(async_bytes.len(), 2 * MIB as u64);
        async_bytes.rewind().await.unwrap();
        assert_eq!(async_bytes.into_vec(u64::MAX).await.unwrap().len(), 2 * MIB);
    }

    #[tokio::test]
    async fn read_only_clones_share_data_but_seek_independently() {
        let _guard = lock_spillover(MIB as u32).await;
        let mut bytes = JumboBytesMut::new(u64::MAX);
        bytes.write_all(&vec![0x33; MIB]).await.unwrap();
        bytes.write_all(&vec![0x44; MIB]).await.unwrap();

        let mut first = bytes.into_read_only().await;
        assert_eq!(first.len(), 2 * MIB);
        let mut second = first.clone();

        first.seek(SeekFrom::Start(0)).await.unwrap();
        second.seek(SeekFrom::Start(MIB as u64)).await.unwrap();

        let mut a = [0u8; 1];
        let mut b = [0u8; 1];
        first.read_exact(&mut a).await.unwrap();
        second.read_exact(&mut b).await.unwrap();
        assert_eq!((a[0], b[0]), (0x33, 0x44), "clones must not share a cursor");
    }
}
